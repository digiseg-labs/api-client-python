...
class AccountSubscriptionPaymentConfiguration(BaseModel):
    """
    Describes the payment configuration of an account subscription
    """ # noqa: E501
    platform: StrictStr
    timing: StrictStr = Field(description="Is the subscription pre-paid or post-paid?")
    stripe_subscription_id: Optional[StrictStr] = None
    stripe_subscription_item_id: Optional[StrictStr] = None
    __properties: ClassVar[List[str]] = ["platform", "timing", "stripe_subscription_id", "stripe_subscription_item_id"]

    @field_validator('platform')
    def platform_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['stripe', 'manual']):
            raise ValueError("must be one of enum values ('stripe', 'manual')")
        return value

    @field_validator('timing')
    def timing_validate_enum(cls, value):
        """Validates the enum"""
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of AccountSubscriptionPaymentConfiguration from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of AccountSubscriptionPaymentConfiguration from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "stripe_subscription_item_id": obj.get("stripe_subscription_item_id")
        })
        return _obj
