...
class AccountSubscriptionCreation(BaseModel):
    """
    AccountSubscriptionCreation
    """ # noqa: E501
    id: StrictStr = Field(description="The ID of the subscription")
    start_date: date = Field(description="Start date (inclusive) of the subscription")
    end_date: Optional[date] = Field(default=None, description="End date (inclusive) of the subscription")
    is_active: StrictBool = Field(description="Is the subscription currently active or not?")
    cancelled_at: Optional[datetime] = Field(default=None, description="Time of cancelling the subscription, if it has been cancelled. Note that a cancelled subscription may still be active, if it has been prepaid for the current period. ")
    actual_price: Optional[SubscriptionPrice] = None
    payment_configuration: Optional[AccountSubscriptionPaymentConfiguration] = None
    plan_id: StrictStr = Field(description="The ID of the plan to subscribe to")
    __properties: ClassVar[List[str]] = ["id", "start_date", "end_date", "is_active", "cancelled_at", "actual_price", "payment_configuration", "plan_id"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of AccountSubscriptionCreation from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of AccountSubscriptionCreation from a dict"""
        if obj is None:
            return None
